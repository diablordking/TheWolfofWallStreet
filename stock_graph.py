...
def graph_addons(axs, Date, data, names):
    ...
    for i in range(alen):
        ...
        if itype == 'dollarvolume':
            ...
            data = pd.read_csv('googleticket1.csv')
            #data = data.loc[data["ticket"] =="GOOGL"]
            print data
...
def graph_adl(ax, Date, data, label, attr):
    print '--'
    print "stock_graph.graph_bb()", label, attr
    ts = pd.Series(data["a"], data["Date"])
    mydata = ts
    print mydata[:10]
    print mydata[-10:]

    # what color to use: http://matplotlib.org/examples/color/named_colors.html
    ax.plot(np.arange(1, 1500), data["a"], color='purple', lw=2, label=label)
    #ax.set_yticks([])
    #print "Transnun  = "+ax.transAxes
    ax.text(0.025, 0.95, label, va='top',  fontsize=12)
    #ax.plot(np.arange(0, 1498), data["a"])
    # ax.plot(np.arange(1,4730), mydata, '*',label=label)
    ax.set_yticks([])

    #plt.text(0.050, 0.95, label, va='top', fontsize=12)
    for label in ax.get_xticklabels():
        label.set_visible(False)

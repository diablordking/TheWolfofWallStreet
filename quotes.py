...
def get_quote_now(symbol):
    ...
    quoteobj = json_loads_byteified(json.loads(quote))
...
if __name__ == '__main__':
    ...
    start = pd.to_datetime(startstr, format = "%Y%m%d")
    st1 = pd.to_datetime(st1str, format = "%Y%m%d")
    st2 = datetime.datetime.strptime(st2str,'%Y%m%d') #both works
    sd = datetime.datetime.strptime(singledaystr, '%Y%m%d')
